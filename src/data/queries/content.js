...
// Extract 'front matter' metadata(yaml string to object) and generate HTML
const parseContent = (path, fileContent, extension) => {
	const fmContent = fm(fileContent);	// fmContent is an object and fileContent need to be a yaml string
	let htmlContent;
	switch (extension) {
		case '.jade':
...
async function resolveExtension(path, extension) {
	...
	// if file not exit redirection to index.extension and if index not exit return fail about the request
	...
	// find the file then return success and the fileName with absolute path.
...
const content = {
	...
	async resolve({request}, {path}) {
		...
		// get the yaml string from fileName with absolute string
